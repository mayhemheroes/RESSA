...
# Step 1: Run MTV triage on the sample file 20 times to detect intermittent crashes
echo "Step 1 - Running initial triage (20 attempts to detect intermittent crashes)..."
...
if [ "$FIRST_CRASHED_ANY" = true ]; then
    ...
    echo "Initial triage: PASSED all 20 attempts (exit code: $FIRST_TRIAGE_EXIT_CODE)"
fi
...
# Step 5: Run MTV triage again 20 times to detect intermittent crashes
echo "Step 5 - Running second triage (20 attempts to detect intermittent crashes)..."
...
if [ "$SECOND_CRASHED_ANY" = true ]; then
    ...
    echo "Second triage: PASSED all 20 attempts (exit code: $SECOND_TRIAGE_EXIT_CODE)"
fi
